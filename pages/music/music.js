...
Page({
  data: {
    ...
    bstop: true,
    playerId:0,
    playerSrc: '../../public/image/icon/playing.png',
    playerLrc:'',
  ...
  addData: function (fn) {
    ...
    if (this.data.hidden) {
      ...
      wx.request({
        url: 'http://tingapi.ting.baidu.com/v1/restserver/ting?type=16&method=baidu.ting.billboard.billList',
        data: {
          type: 1,
  ...
  playmusic: function (id, fn) {
    ...
    wx.request({
      url: 'http://tingapi.ting.baidu.com/v1/restserver/ting?type=16&method=baidu.ting.song.play',
      data: { songid: id },
      method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      // header: {}, // 设置请求的 header
      success: function (res) {
  ...
  onAudioChange:function(event){
    //歌曲时间发生变化时触发;
    var playerTime = parseInt(event.detail.currentTime);
    var that = this;
    var pro = parseInt(event.detail.currentTime / event.detail.duration * 100)
